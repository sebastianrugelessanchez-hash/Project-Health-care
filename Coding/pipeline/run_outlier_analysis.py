...
def main():
    ...
    for col in df.columns:
        col_normalized = str(col).lower().replace(' ', '').replace('-', '').replace('_', '')
        for uc in USABLE_COLUMNS:
            uc_normalized = uc.lower().replace(' ', '').replace('-', '').replace('_', '')
            if col_normalized == uc_normalized:
                usable_cols.append(col)
                break

    df_analysis = df[usable_cols] if usable_cols else df.select_dtypes(include=['number'])
